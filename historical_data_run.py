...
STATE_FILE = "historical_run_state.json"

# --- Funções para carregar e guardar o estado ---
def load_state():
    """Carrega o estado da execução a partir de um ficheiro JSON."""
...
def save_state(state):
    """Guarda o estado da execução num ficheiro JSON."""
...
class MercadoLivreAdsCollector:
    def __init__(self, access_token):
        ...
        self.timeout = 30  # Timeout padrão de 30 segundos para as chamadas
    ...
    def get_ads_summary_metrics(self, advertiser_id, date_from, date_to):
        ...
        try:
            params = {"date_from": date_from, "date_to": date_to, "metrics_summary": "true", "metrics": "cost,acos,direct_amount,indirect_amount,total_amount"}
...
def export_to_google_sheets(df, sheet_name, worksheet_name, google_creds):
    ...
    try:
        ...
        try:
            ...
            # Garante que o cabeçalho seja escrito se a aba for nova
        ...
        logger.info(f"SUCESSO: As linhas foram adicionadas na aba '{worksheet_name}'.")
        return spreadsheet.url
    except Exception as e:
        logger.error(f"ERRO AO EXPORTAR PARA '{worksheet_name}': {e}", exc_info=True)
        return None

# --- Lógica Principal da Automação Histórica (EM ORDEM INVERSA) ---
def main():
    logger.info("Iniciando a execução da extração de dados históricos (EM ORDEM INVERSA).")
    ...
    # --- DEFINIÇÃO DO PERÍODO HISTÓRICO ---
    limit_date_past = datetime(2024, 1, 1)  # Data mais antiga a ser buscada
    start_date_today = datetime.now()      # Ponto de partida é sempre hoje
    ...
    for index, client_info in clients_df.iterrows():
        ...
        # --- LÓGICA DE CONTINUAÇÃO INVERTIDA ---
        last_processed_date_str = state.get(client_name)
        if last_processed_date_str:
            # Continua a partir do dia anterior ao último processado
            start_date_for_run = datetime.strptime(last_processed_date_str, '%Y-%m-%d') - timedelta(days=1)
            logger.info(f"Continuando extração retroativa a partir de {start_date_for_run.strftime('%Y-%m-%d')} para o cliente {client_name}.")
        else:
            # Primeira vez, começa a partir de hoje
            start_date_for_run = start_date_today
            logger.info(f"Iniciando extração retroativa a partir de hoje ({start_date_for_run.strftime('%Y-%m-%d')}) para o cliente {client_name}.")
        
        if start_date_for_run.date() < limit_date_past.date():
            logger.info(f"Cliente {client_name} já tem todo o histórico processado até o limite. Pulando.")
            continue

        # --- GERAÇÃO DO PERÍODO INVERSO ---
        date_range = pd.date_range(start=limit_date_past, end=start_date_for_run, tz=brasil_timezone)
        reversed_date_range = sorted(date_range, reverse=True)
        ...
        try:
            ...
            # --- Loop por data (do mais recente para o mais antigo) ---
            for single_date in reversed_date_range:
                date_str = single_date.strftime('%Y-%m-%d')
                logger.info(f"--- Coletando dados para o dia: {date_str} ---")
                ...
                business_metrics = {}
                if user_id:
                    business_metrics = collector.get_orders_metrics(user_id, date_str, date_str)

                ads_metrics = collector.get_ads_summary_metrics(advertiser_id, date_str, date_str) or {}
                
                consolidated_data = {
                    "data_geracao": timestamp_geracao,
                    "periodo_consulta": periodo_consulta,
                ...
                # Exporta a linha de dados consolidados, garantindo que algo seja sempre escrito.
                export_to_google_sheets(df_consolidated, "Histórico de Vendas Meli - 2024", "Dados Consolidados", google_creds)
                
                campaigns_data = collector.get_all_campaigns_paginated(advertiser_id, date_str, date_str)
                if campaigns_data:
                    ...
                    export_to_google_sheets(df_analysis[colunas_existentes], "Histórico de Vendas Meli - 2024", "Analise de Campanhas", google_creds)
                else:
                    logger.info(f"Nenhuma campanha encontrada para {client_name_from_api} no dia {date_str}.")

                # Atualiza o estado com o dia que acabou de ser processado com sucesso
                state[client_name] = date_str
                save_state(state)

        except Exception as e:
            logger.error(f"ERRO INESPERADO ao processar {client_name}. O progresso até o dia anterior foi guardado. Erro: {e}", exc_info=True)
            continue
            
    logger.info("\nExecução da extração histórica finalizada.")
